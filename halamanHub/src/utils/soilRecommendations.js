// HalamanHub — Soil recommendations (client-side copy)
//
// Same rule-based logic as server/utils/soilRecommendations.js, kept here
// so the dashboard can compute recommendations directly from a LIVE sensor
// reading (received over Socket.io / the USB serial bridge) without needing
// any backend call — this is what makes recommendations work in offline
// mode, when MongoDB (and therefore /api/dashboard/summary) is unreachable.
//
// general guidance, not crop-specific — the system doesn't track which
// crop is planted.

export function getSoilRecommendations({ ph, ec, nitrogen, phosphorus, potassium, temperature, humidity }) {
  const recs = [];
