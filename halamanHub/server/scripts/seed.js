...
async function seed() {
  ...
  if (userCount === 0) {
    if (!ADMIN_USERNAME || !ADMIN_PASSWORD) {
      console.error('✗ No users found and ADMIN_USERNAME/ADMIN_PASSWORD not set in .env — skipping user seed.');
    } else {
      const user = new User({
        name: 'Admin',
        email: `${ADMIN_USERNAME}@halamanhub.ph`,
      ...
      await user.save();
      console.log(`✓ Seeded 1 admin user (login: ${ADMIN_USERNAME} / ${ADMIN_PASSWORD})`);
    }
  } else {
  ...
  if (orderCount === 0) {
    ...
    await Order.insertMany([
      ...
      { orderNumber: '#ORD-0007', customer: 'Liza Torres',     product: 'Pechay (2 bundles)',      quantity: 2, amount: 110, status: 'cancelled',  payment: 'paid', orderDate: days(3), statusHistory: [{ status: 'pending',    note: '',                                 changedAt: days(3), changedBy: 'admin' }, { status: 'cancelled',  note: 'Customer requested cancellation', changedAt: days(2), changedBy: 'admin' }] },
  ...
  if (sensorCount === 0) {
    ...
    console.log(`✓ Sensors already exist (${sensorCount}) — skipping`);
  }

  // Irrigation zones
  const zoneCount = await IrrigationZone.countDocuments();
  if (zoneCount === 0) {
    ...
    console.log(`✓ Irrigation zones already exist (${zoneCount}) — skipping`);
  }
  ...
  if (scheduleCount === 0) {
    ...
    console.log(`✓ Schedules already exist (${scheduleCount}) — skipping`);
  }
